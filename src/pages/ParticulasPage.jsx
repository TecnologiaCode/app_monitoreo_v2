...
const ParticulasPage = () => {
    ...
    try {
        ...
        for (const r of registrosSeleccionados) {
            ...
            dataForPdf.push({
                ...
                area: r.area || 'N/A', // Ahora sí usamos el campo 'area'
                puesto: r.puesto_trabajo,
                codigo: codigo,
                fechaHora: `${formatFechaUTC(r.measured_at)} - ${formatHoraUTC(r.measured_at)}`
            });
            supabaseTasks.push(
                supabase.from(PARTICULAS_TABLE).update({ selected_image_index: finalIdx }).eq('id', r.id)
            );
            i++;
        }
  ...
  const fetchEquiposInfo = async (equipoIds) => {
    ...
    if (!Array.isArray(list) || !list.length) { setEquiposInfo([]); return; }
    try {
      const { data, error } = await supabase.from('equipos').select('id, nombre_equipo, modelo, serie').in('id', list);
      if (error) throw error;
      setEquiposInfo(data || []);
    } catch (err) {
  ...
  const fetchRegistros = async () => {
    setLoading(true);
    try {
      const cols = 'id, created_at, proyecto_id, monitoreo_id, measured_at, area, puesto_trabajo, temperatura_c, hr_percent, pm25_values, pm25_prom, pm10_values, pm10_prom, pts_values, pts_prom, observaciones, image_urls, location, created_by, selected_image_index';
      ...
      else { setRegistros([]); setLoading(false); return; }
      query = query.order('measured_at', { ascending: true, nullsFirst: true }).order('created_at', { ascending: true });
        ...
        const normalizeArray = (val) => {
          ...
          if (typeof val === 'string' && val.trim() !== '') {
            try {
              ...
              if (Array.isArray(parsed)) return parsed;
            } catch (_) { return val.split(',').map((s) => s.trim()); }
          }
          return [];
        };
        let imgs = [];
  ...
  const fetchUsersByIds = async (ids) => {
    try {
      ...
      if (error) return;
      const dict = {};
      (data || []).forEach((u) => { dict[u.id] = u.nombre_completo || u.username || u.id; });
      setUsersById(dict);
  ...
  useEffect(() => {
    fetchRegistros();
    const channel = supabase.channel('rt-particulas').on('postgres_changes', { event: '*', schema: 'public', table: PARTICULAS_TABLE }, () => fetchRegistros()).subscribe();
  ...
  const handleDelete = (record) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const { error } = await supabase.from(PARTICULAS_TABLE).delete().eq('id', record.id);
          if (error) throw error;
  ...
  const parseNumberArray = (txt) => {
    ...
    const parts = txt.split(',').map((s) => s.trim()).filter(Boolean).map((n) => Number(n));
  ...
  const handleAddOk = async () => {
    ...
    try {
      ...
      if (!monitoreoId && !projectId) { message.error('Error de contexto.'); return; }
      const payload = {
        ...
        image_urls: values.image_urls && values.image_urls.trim() !== '' ? values.image_urls.split(',').map((s) => s.trim()).filter(Boolean) : null,
    ...
    } catch (err) { console.error('Error add:', err); message.error('No se pudo agregar.'); } finally { setSaving(false); }
  };
  ...
  const handleEditOk = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from(PARTICULAS_TABLE).update(updateData).eq('id', selectedRegistro.id);
      if (error) throw error;
      message.success('Registro actualizado.');
      setIsFormModalVisible(false);
    } catch (err) { console.error('Error edit:', err); message.error('No se pudo actualizar.'); } finally { setSaving(false); }
  };
  ...
  const exportToExcel = () => {
    try {
      ...
      const headerStyle = { font: { bold: true }, alignment: { vertical: 'center', horizontal: 'center', wrapText: true }, fill: { fgColor: { rgb: 'D9E1F2' } }, border: { top: { style: 'thin', color: { rgb: '000000' } }, bottom: { style: 'thin', color: { rgb: '000000' } }, left: { style: 'thin', color: { rgb: '000000' } }, right: { style: 'thin', color: { rgb: '000000' } } } };
      const cellStyle = { alignment: { vertical: 'center', horizontal: 'left', wrapText: true }, border: { top: { style: 'thin', color: { rgb: '000000' } }, bottom: { style: 'thin', color: { rgb: '000000' } }, left: { style: 'thin', color: { rgb: '000000' } }, right: { style: 'thin', color: { rgb: '000000' } } } };

      const empresa = proyectoInfo?.nombre || '';
      const firstReg = registros && registros.length ? registros[0] : null;
      const fechaInicio = firstReg?.measured_at ? dayjs(firstReg.measured_at).utc().format('DD/MM/YYYY') : (proyectoInfo?.measured_at ? dayjs(proyectoInfo.measured_at).utc().format('DD/MM/YYYY') : '');
      const fechaFin = '';

      wsData.push([{ v: 'PLANILLA DE MEDICIÓN DE PARTÍCULAS SUSPENDIDAS', s: { font: { bold: true, sz: 14 }, alignment: { vertical: 'center', horizontal: 'center' } } }]);
      wsData.push([{ v: 'INSTALACIÓN:', s: titleStyle }, { v: empresa, s: cellStyle }, '', { v: 'FECHA DE INICIO DEL MONITOREO:', s: titleStyle }, { v: fechaInicio, s: cellStyle }]);
      wsData.push([{ v: 'FECHA DE FINALIZACIÓN DEL MONITOREO:', s: titleStyle }, { v: fechaFin, s: cellStyle }, '', { v: 'TIPO DE MONITOREO:', s: titleStyle }, { v: monitoreoInfo?.tipo_monitoreo || 'PARTÍCULAS', s: cellStyle }]);
      wsData.push(['', '', '', '', '']);

      wsData.push([
        { v: 'N°', s: headerStyle },
        { v: 'Área', s: headerStyle }, // NUEVA COLUMNA EXCEL
      ...
      registros.forEach((r, idx) => {
        ...
        wsData.push([
          { v: idx + 1, s: cellStyle },
          { v: r.area || '', s: cellStyle }, // DATO AREA
          { v: r.puesto_trabajo || '', s: cellStyle },
          { v: r.measured_at ? dayjs(r.measured_at).utc().format('DD/MM/YYYY') : '', s: cellStyle },
          { v: r.measured_at ? dayjs(r.measured_at).utc().format('HH:mm') : '', s: cellStyle },
          { v: r.temperatura_c ?? '', s: cellStyle },
          { v: r.hr_percent ?? '', s: cellStyle },
          { v: pm10 != null ? Number(pm10).toFixed(2) : '', s: cellStyle },
          { v: pm25 != null ? Number(pm25).toFixed(2) : '', s: cellStyle },
          { v: r.pts_prom != null ? Number(r.pts_prom).toFixed(2) : '', s: cellStyle },
          { v: '10(I)', s: cellStyle },
          { v: '3(R)', s: cellStyle },
      ...
      // MERGES REAJUSTADOS (todo +1 columna a la derecha despues de N°)
      ws['!merges'] = [
        ...
        { s: { r: 4, c: 1 }, e: { r: 5, c: 1 } }, // Area (NUEVO MERGE)
    ...
    } catch (err) { console.error(err); message.error('No se pudo exportar.'); }
  };
  ...
  /* ================ COLUMNAS TABLA ================ */
  const columns = [
    { title: 'N°', key: 'numero', width: 40, fixed: 'left', render: (_, __, i) => (currentPage - 1) * pageSize + i + 1 },
    { 
      title: 'Fecha', 
      ...
      key: 'measured_date', 
        // ✅ Permite ordenar ascendente/descendente por fecha
            ...
            width: 120, render: (v) => formatFechaUTC(v),
      
    },
    { title: 'Hora de Medición', dataIndex: 'measured_at', key: 'measured_at', width: 140, sorter: (a, b) => dayjs(a.measured_at).unix() - dayjs(b.measured_at).unix(), render: (v) => formatHoraUTC(v) },
    // NUEVA COLUMNA EN UI
    { title: 'Área', dataIndex: 'area', width: 150, ellipsis: true },
    { title: 'Puesto de Trabajo Evaluado', dataIndex: 'puesto_trabajo', key: 'puesto_trabajo', width: 220 },
    { title: 'Temperatura °C', dataIndex: 'temperatura_c', key: 'temperatura_c', width: 120 },
    { title: 'H.R. %', dataIndex: 'hr_percent', key: 'hr_percent', width: 100 },
    { title: 'Resultados  µg/m³', children: [
        { title: 'PM 10', dataIndex: 'pm10_prom', key: 'pm10_prom', width: 100, render: (v) => (v != null ? Number(v).toFixed(2) : '') },
        { title: 'PM 2.5', dataIndex: 'pm25_prom', key: 'pm25_prom', width: 100, render: (v) => (v != null ? Number(v).toFixed(2) : '') },
        { title: 'PTS', dataIndex: 'pts_prom', key: 'pts_prom', width: 100, render: (v) => (v != null ? Number(v).toFixed(2) : '') },
    ] },
    { title: 'Imágenes', dataIndex: 'image_urls', key: 'image_urls', width: 130, render: (imgs) => {
    ...
    { title: 'Observaciones', dataIndex: 'observaciones', key: 'observaciones', width: 200, ellipsis: true },
    { title: 'Registrado por', dataIndex: 'created_by', key: 'created_by', width: 190, fixed: 'right', render: (v) => usersById[v] || v },
    { title: 'Acciones', key: 'acciones', fixed: 'right', width: 110, render: (_, record) => (
        <Space size="small">
  ...
  const headerFechaInicio = firstRegistro?.measured_at ? dayjs(firstRegistro.measured_at).utc().format('DD/MM/YYYY') : (proyectoInfo?.created_at ? dayjs(proyectoInfo.created_at).utc().format('DD/MM/YYYY') : 'N/A');
  const headerFechaFin = '';
  const safeEquipos = Array.isArray(equiposInfo) ? equiposInfo : [];
  const headerEquipos = safeEquipos.length > 0 ? safeEquipos.map((eq) => eq.nombre_equipo || 's/n').join(', ') : 'Ninguno';
  const headerModelos = safeEquipos.length > 0 ? safeEquipos.map((eq) => eq.modelo || 's/n').join(', ') : 'N/A';
  const headerSeries = safeEquipos.length > 0 ? safeEquipos.map((eq) => eq.serie || 's/n').join(', ') : 'N/A';
  ...
  return (
    <>
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
        <Col><Title level={2} style={{ color: PRIMARY_BLUE, marginBottom: 0 }}>Monitoreo de Partículas</Title></Col>
        <Col>
          <Space>
      ...
      <Spin spinning={loadingHeader}>
        <Descriptions bordered size="small" column={2} style={{ marginBottom: 15 }}>
          ...
          <Descriptions.Item label="FECHA DE FINALIZACIÓN">{headerFechaFin}</Descriptions.Item>
      ...
      <Row justify="space-between" align="middle" style={{ marginTop: 12 }}>
        <Col>{(() => { const mostradosHastaAqui = Math.min(currentPage * pageSize, totalFiltered); return (<Text type="secondary">Registros {mostradosHastaAqui} de {totalFiltered}</Text>); })()}</Col>
      ...
      <Modal title={selectedRegistro ? 'Editar registro de partículas' : 'Agregar registro de partículas'} open={isFormModalVisible} onOk={handleSaveAndGenerate} onCancel={() => setIsFormModalVisible(false)} confirmLoading={saving} width={760} destroyOnClose footer={[
        <Button key="back" onClick={() => setIsFormModalVisible(false)}>Cancelar</Button>,
        <Button key="submit" type="primary" loading={saving} onClick={() => form.submit()}>Guardar</Button>
      ]}>
        <Form form={form} layout="vertical" preserve={false} key={selectedRegistro ? `edit-${selectedRegistro.id}` : 'add'} onFinish={selectedRegistro ? handleEditOk : handleAddOk}
          initialValues={
            selectedRegistro ? {
                measured_at: selectedRegistro.measured_at ? dayjs(selectedRegistro.measured_at).utc().format('YYYY-MM-DD HH:mm') : '',
                area: selectedRegistro.area || '', // CAMPO AREA
                puesto_trabajo: selectedRegistro.puesto_trabajo,
                temperatura_c: selectedRegistro.temperatura_c ? Number(selectedRegistro.temperatura_c) : undefined,
                hr_percent: selectedRegistro.hr_percent ? Number(selectedRegistro.hr_percent) : undefined,
                pm25_values: Array.isArray(selectedRegistro.pm25_values) ? selectedRegistro.pm25_values.join(', ') : '',
                pm25_prom: selectedRegistro.pm25_prom,
                pm10_values: Array.isArray(selectedRegistro.pm10_values) ? selectedRegistro.pm10_values.join(', ') : '',
                pm10_prom: selectedRegistro.pm10_prom,
                pts_values: Array.isArray(selectedRegistro.pts_values) ? selectedRegistro.pts_values.join(', ') : '',
                pts_prom: selectedRegistro.pts_prom,
                observaciones: selectedRegistro.observaciones,
                image_urls: Array.isArray(selectedRegistro.image_urls) ? selectedRegistro.image_urls.join(', ') : (selectedRegistro.image_urls || ''),
                location: selectedRegistro.location || '',
            } : { measured_at: dayjs().utc().format('YYYY-MM-DD HH:mm') }
          }
        >
          <Form.Item name="measured_at" label="Hora / fecha de medición"><Input placeholder="YYYY-MM-DD HH:mm o solo HH:mm" /></Form.Item>
          {/* CAMPO AREA */}
          <Form.Item name="area" label="Área" rules={[{ required: true, message: 'Campo obligatorio' }]}><Input /></Form.Item>
          <Form.Item name="puesto_trabajo" label="Puesto de Trabajo Evaluado" rules={[{ required: true, message: 'Campo obligatorio' }]}><Input /></Form.Item>
          <Form.Item name="temperatura_c" label="Temperatura °C" rules={[{ required: true, message: 'Campo obligatorio' }]}><InputNumber min={-10} max={60} style={{ width: '100%' }} /></Form.Item>
          <Form.Item name="hr_percent" label="H.R. %" rules={[{ required: true, message: 'Campo obligatorio' }]}><InputNumber min={0} max={100} style={{ width: '100%' }} /></Form.Item>
          <Form.Item label="Promedios (µg/m³)" style={{ marginBottom: 4 }} />
          <Row gutter={8}>
            <Col span={8}><Form.Item name="pm10_prom" label="PM 10 (prom)" rules={[{ required: true, message: 'Campo obligatorio' }]}><InputNumber min={0} step={0.01} style={{ width: '100%' }} /></Form.Item></Col>
            <Col span={8}><Form.Item name="pm25_prom" label="PM 2.5 (prom)" rules={[{ required: true, message: 'Campo obligatorio' }]}><InputNumber min={0} step={0.01} style={{ width: '100%' }} /></Form.Item></Col>
            <Col span={8}><Form.Item name="pts_prom" label="PTS (prom)" rules={[{ required: true, message: 'Campo obligatorio' }]}><InputNumber min={0} step={0.01} style={{ width: '100%' }} /></Form.Item></Col>
          </Row>
          <Form.Item label="Valores originales (opcional) separados por coma" style={{ marginBottom: 4 }} />
          <Form.Item name="pm10_values" label="PM 10 values"><Input placeholder="Ej: 2, 3, 1" /></Form.Item>
          <Form.Item name="pm25_values" label="PM 2.5 values"><Input placeholder="Ej: 1, 2, 3" /></Form.Item>
          <Form.Item name="pts_values" label="PTS values"><Input placeholder="Ej: 4, 3, 2" /></Form.Item>
          <Form.Item name="observaciones" label="Observaciones"><Input.TextArea rows={3} /></Form.Item>
          <Form.Item name="image_urls" label="URLs de imágenes (coma separadas)"><Input.TextArea rows={2} placeholder="https://... , https://..." /></Form.Item>
          <Form.Item name="location" label="Ubicación (texto o JSON)"><Input placeholder={'Ej: -16.5, -68.1'} /></Form.Item>
        </Form>
      </Modal>
      ...
      <Modal open={imageViewerOpen} onCancel={() => setImageViewerOpen(false)} footer={imageViewerList.length > 1 ? [<Button key="prev" onClick={() => setImageViewerIndex((prev) => (prev - 1 + imageViewerList.length) % imageViewerList.length)}>Anterior</Button>,<Button key="next" type="primary" onClick={() => setImageViewerIndex((prev) => (prev + 1) % imageViewerList.length)}>Siguiente</Button>,] : null} width={720} title="Imagen de la medición">
        {imageViewerList.length ? (<div style={{ textAlign: 'center' }}><img src={imageViewerList[imageViewerIndex]} alt="partículas" style={{ maxWidth: '100%', maxHeight: '60vh', objectFit: 'contain' }} /><div style={{ marginTop: 8 }}>{imageViewerIndex + 1} / {imageViewerList.length}</div></div>) : (<Text type="secondary">Sin imagen.</Text>)}
      </Modal>
                      ...
                      {registros.filter(r => getImagesArray(r).length > 0).map((r) => {
                          ...
                          return (
                              <div key={r.id} style={{ width: '23%', border: isSelected ? '1px solid #ddd' : '1px dashed #999', opacity: isSelected ? 1 : 0.5, padding: '8px', borderRadius: '8px', display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor: '#fafafa', position: 'relative' }}>
                                  ...
                                  <Text strong style={{ fontSize: 12 }}>{monitoreoInfo?.tipo_monitoreo || 'Partículas'}</Text>
              ...
              {pdfStep === 'view' && (
                  <PDFViewer width="100%" height="100%" showToolbar={true}>
                      <ReporteFotografico 
                          ...
                          layout={pdfLayout}
                          tituloMonitoreo={monitoreoInfo?.tipo_monitoreo || 'Partículas'} 
                          descripcionProyecto={''}
                      />
                  </PDFViewer>
              )}
