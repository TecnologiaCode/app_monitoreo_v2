...
// ► NUEVO: librerías para generar el ZIP de imágenes
...
const ErgonomiaPage = () => {
    ...
    // Limpia URLs de imágenes (string, JSON, array Postgres, etc.)
    ...
    // Array de imágenes ya normalizado
    ...
    const downloadAllImages = async () => {
        try {
            ...
            const zip = new JSZip();
            const folder = zip.folder('ergonomia');
            ...
            const zipName = `ergonomia_imagenes_${empresaSafe}_${fechaSafe || 'monitoreo'}.zip`;
    ...
    const handleOpenPdf = () => {
        const registrosConFotos = rows.filter(
        ...
        registrosConFotos.forEach((r) => {
            ...
            initialSelections[r.id] =
                savedIndex < imgs.length ? savedIndex : 0;
    ...
    const handleSaveAndGenerate = async () => {
        ...
        try {
            ...
            for (const r of registrosSeleccionados) {
                const imgs = getImagesArray(r);
                const selectedIdx =
                    tempSelections[r.id] !== undefined
                        ? tempSelections[r.id]
                        : 0;
                const finalIdx =
                    selectedIdx < imgs.length ? selectedIdx : 0;
                ...
                dataForPdf.push({
                    ...
                    area: r.area_trabajo,
                    puesto: r.cargo || r.trabajador_nombre,
                    codigo,
                    fechaHora: `${formatFechaUTC(
                        r.measured_at
                    )} - ${formatHoraUTC(r.measured_at)}`
                });
    ...
    const exportToExcel = () => {
        try {
            ...
            rows.forEach((r, idx) => {
                ...
                wsData.push([
                    { v: 'Nombre /Razón Social:', s: sLabel },
                    { v: empresaNombre, s: sYellow }
                ]);
                wsData.push(['']);

                wsData.push([{ v: 'DATOS DEL TRABAJADOR', s: sHeader }]);
                wsData.push([
                    { v: 'Área de Trabajo:', s: sLabel },
                    { v: r.area_trabajo || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Nombre del Trabajador:', s: sLabel },
                    { v: r.trabajador_nombre || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Cargo:', s: sLabel },
                    { v: r.cargo || '', s: sYellow }
                ...
                wsData.push([
                    { v: 'Dolencias:', s: sLabel },
                    { v: r.dolencias || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Actividades:', s: sLabel },
                ...
                wsData.push([
                    { v: 'Peso (Kg):', s: sLabel },
                    { v: r.peso_kg || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Altura (m):', s: sLabel },
                    { v: r.altura_m || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Antigüedad (meses):', s: sLabel },
                    { v: r.antiguedad_meses || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Saturación O2:', s: sLabel },
                    { v: r.o2 || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Presión (Pc):', s: sLabel },
                    { v: r.presion_pc || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Fuerza Agarre (Kg):', s: sLabel },
                    { v: r.fuerza_agarre_kg || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Mano Agarre:', s: sLabel },
                    { v: r.mano_agarre || '', s: sYellow }
                ]);
                wsData.push(['']);

                wsData.push([{ v: 'MANIPULACIÓN DE CARGAS', s: sHeader }]);
                wsData.push([
                    { v: 'Carga Peso (Kg):', s: sLabel },
                    { v: r.carga_peso_kg || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Tipo de Agarre:', s: sLabel },
                    { v: r.carga_agarre || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Frecuencia:', s: sLabel },
                    { v: r.carga_frecuencia || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Distancia (m):', s: sLabel },
                    { v: r.carga_distancia_m || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Ayuda Mecánica:', s: sLabel },
                    { v: r.carga_ayuda || '', s: sYellow }
                ]);
                wsData.push([
                    { v: 'Descripción Carga:', s: sLabel },
                    { v: r.carga_descripcion || '', s: sYellow }
                ]);
                wsData.push(['']);

                wsData.push([{ v: 'OBSERVACIONES', s: sHeader }]);
                wsData.push([
                    { v: r.observaciones || 'Sin observaciones', s: sYellow }
                ]);
                ...
                if (monitoreoId) {
                    ...
                    setHeaderInfo((h) => ({
                        ...
                        fecha: p?.created_at
                            ...
                            : '—',
                        equipo: equipos.length
                            ? equipos
                                  .map((e) => e.nombre_equipo || 's/n')
                                  .join(', ')
                            : '',
                        modelos: equipos.length
                            ? equipos
                                  .map((e) => e.modelo || 's/n')
                                  .join(', ')
                            : '',
                        series: equipos.length
                            ? equipos
                                  .map((e) => e.serie || 's/n')
                                  .join(', ')
                            : '',
    ...
    const fetchRows = async () => {
        ...
        try {
            ...
            if (monitoreoId && projectId) {
                q = q.or(
                    `monitoreo_id.eq.${monitoreoId},proyecto_id.eq.${projectId}`
                );
            } else if (monitoreoId) {
            ...
            const ids = Array.from(
                new Set(mapped.map((m) => m.created_by).filter(Boolean))
            );
            if (ids.length) {
                const { data: profs } = await supabase
                    .from('profiles')
                    .select(
                        'id, username, nombre_completo, email, descripcion, rol, estado'
                    )
                    .in('id', ids);
                const dict = {};
                (profs || []).forEach((u) => {
            ...
            message.error('No se pudo cargar Ergonomía.');
            setRows([]);
        } finally {
            setLoading(false);
        }
    ...
    const payloadFromValues = async (values) => {
        ...
        if (values.horario_medicion) {
            ...
            const base = selected?.measured_at
                ? dayjs(selected.measured_at)
                : dayjs();
            const local = base
                .hour(h)
                .minute(m)
    ...
    const filtered = useMemo(() => {
        ...
        return rows.filter((r) => {
            return (
                (r.area_trabajo &&
                    r.area_trabajo.toLowerCase().includes(s)) ||
                (r.trabajador_nombre &&
                    r.trabajador_nombre.toLowerCase().includes(s)) ||
                (r.cargo && r.cargo.toLowerCase().includes(s)) ||
                (r.actividades &&
                    r.actividades.toLowerCase().includes(s)) ||
                (r.observaciones &&
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'measured_date',
            sorter: (a, b) =>
                dayjs(a.measured_at).unix() -
                dayjs(b.measured_at).unix(),
        ...
        {
            title: 'Presión (Pc)',
            dataIndex: 'presion_pc',
            key: 'presion_pc',
            width: 80
        },
        {
            title: 'Fuerza Agarre (Kg)',
            dataIndex: 'fuerza_agarre_kg',
            key: 'fuerza_agarre_kg',
            width: 100
        },
        {
            title: 'Mano Agarre',
            dataIndex: 'mano_agarre',
            key: 'mano_agarre',
            width: 100
        },
        {
            title: 'Carga: Peso (Kg)',
            dataIndex: 'carga_peso_kg',
            key: 'carga_peso_kg',
            width: 100
        },
        {
            title: 'Carga: Agarre',
            dataIndex: 'carga_agarre',
            key: 'carga_agarre',
            width: 150,
            ellipsis: true
        },
        {
            ...
            dataIndex: 'carga_frecuencia',
            key: 'carga_frecuencia',
            width: 150,
            ellipsis: true
        },
        {
            title: 'Carga: Distancia (m)',
            dataIndex: 'carga_distancia_m',
            key: 'carga_distancia_m',
            width: 100
        },
        {
            title: 'Carga: Ayuda',
            dataIndex: 'carga_ayuda',
            key: 'carga_ayuda',
            width: 150,
            ellipsis: true
        },
        {
            title: 'Carga: Descripción',
            dataIndex: 'carga_descripcion',
            key: 'carga_descripcion',
            width: 250,
            ellipsis: true
        },
        {
            ...
            render: (imgs) => {
                ...
                if (!list.length)
                    return <Text type="secondary">Ninguna</Text>;
                return (
                    <Button
                        type="link"
                        icon={<EyeOutlined />}
        ...
        },
        {
            ...
            render: (v) => {
                ...
                return display ? (
                    <Tooltip title={display}>{display}</Tooltip>
                ) : (
                    <Text type="secondary">
                        {v.slice(0, 8)}...
                    </Text>
        ...
        <>
            ...
            >
                ...
                <Col>
                    <Space>
                        ...
                        {/* NUEVO BOTÓN: Descargar todas las imágenes */}
            ...
            <Spin spinning={loadingHeader}>
                ...
                >
                    ...
                    <Descriptions.Item label="FECHA DE MONITOREO">
                        {headerInfo.fecha}
                    </Descriptions.Item>
            ...
            <Modal
                title={
                    selected
                        ? 'Editar Registro de Ergonomía'
                        : 'Agregar Registro de Ergonomía'
                }
                open={isFormOpen}
                onOk={onOkForm}
            ...
            >
                <Form
                    ...
                    initialValues={
                        selected
                            ? {
                                  ...selected,
                                  horario_medicion: selected.measured_at
                                      ? dayjs(selected.measured_at).local()
                                      : null,
                                  actividades:
                                  ...
                                  image_urls: Array.isArray(
                                      ...
                                      ? selected.image_urls.join(', ')
                                      : selected.image_urls || '',
                                  location:
                                      typeof selected.location === 'object'
                                          ? JSON.stringify(
                                                selected.location
                                            )
                                          : selected.location || ''
                ...
                >
                    ...
                    {/* Aquí irían el resto de campos del formulario
                        (los mantienes tal como ya los tienes en tu proyecto) */}
                    ...
                    >
                        <Input placeholder="Ej: {&quot;easting&quot;: 792865.73, ...}" />
                    </Form.Item>
                </Form>
            </Modal>
                    ...
                    <div style={{ textAlign: 'center' }}>
                        <img
                            ...
                            onError={(e) => {
                                ...
                                e.target.src =
                                    'https://via.placeholder.com/400x300?text=Error+Cargando+Imagen';
                            }}
                        />
                    ...
                    {pdfStep === 'view' && (
                        ...
                        >
                            <ReporteFotografico
                                data={pdfData}
                                empresa={
                                    headerInfo.descripcion_proyecto ||
                                    'SIN DESCRIPCIÓN'
                                }
                                layout={pdfLayout}
                                tituloMonitoreo={
